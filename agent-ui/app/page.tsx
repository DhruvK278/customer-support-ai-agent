...
import { FiGithub, FiPlay, FiEye, FiFilter } from "react-icons/fi";
...
const statusBadgeClass = (status: string) => {
  switch (status) {
    ...
    case "Manually Resolved":
      return "status-manually-resolved";
    default:
      return "badge-default";
  }
};
...
export default function Home() {
  ...
  const [viewMode, setViewMode] = useState<"All" | "Pending Review">("All");
        ...
        <div className="flex flex-col w-full xl:w-5/12 p-6 lg:p-8 min-h-[40vh]">
          ...
          <div className="flex items-center gap-2 mb-4 animate-fade-in-up">
            ...
            <button
              ...
              className={`filter-pill ${viewMode === "All" ? "filter-pill-active" : ""}`}
            >
              All Tickets
            </button>
            <button
              onClick={() => setViewMode("Pending Review")}
              className={`filter-pill ${viewMode === "Pending Review" ? "filter-pill-active" : ""}`}
            >
              Pending Review
            </button>
          </div>
          ...
          >
            {loading ? (
              ...
              data
                .filter((item) => viewMode === "All" || item.status === "Pending Review")
                .map((item, index) => (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex items-center justify-between w-full gap-3">
                      {/* Issue */}
                      <div className="flex flex-col min-w-0 w-[25%]">
                      ...
                      <div className="flex flex-col w-[22%]">
                        ...
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full w-fit ${statusBadgeClass(item.status)}`}>
                          {item.status || "—"}
                        </span>
                      </div>
